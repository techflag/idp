...
def _build_record_collection_evidence_package(
    *,
        ...
        for block_index, block in enumerate(blocks):
            ...
            selected_evidence_summary.append(
                _field_list_evidence_summary_item(
                    ...
                    source_type="table",
                    table_meta=table_meta,
                    match_reason="record_table_structured_rows",
                    uncertainties=uncertainties,
                )
            )
            selected_blocks.append(compact_block)
    ...
    if not selected_pages and selected_source_text:
    ...
    if total_table_rows > selected_table_rows:
        selection_warnings.append(
            f"记录表证据以结构化行进入模型：{selected_table_rows}/{total_table_rows} 行，完整 facts 已保留用于本地生成。"
        )
...
def _compact_record_collection_table_block(
    *,
    ...
    selected_indexes = list(range(original_row_count))
    compact_grid = _slice_table_grid_rows(
        table_grid=table_grid,
        selected_indexes=selected_indexes,
        row_reason="selected_content_full_record_table",
    )
    compact_grid["evidencePackage"] = True
    compact_grid["expansionLevel"] = "selected_full"
    compact_block = {
        **block,
        "tableGrid": compact_grid,
        "sourceRef": source_ref,
...
class PromptPipelineService:
    ...
    def _execute_single_extraction_skill_run(
        ...
        *,
        ...
        application_scoped_run = isinstance((run.schemaDefinition or {}).get("application"), dict)
        ...
        try:
            ...
            persist_run_artifact = (
                not application_scoped_run
                and bool((run.schemaDefinition or {}).get("persistRunArtifact", True))
            )
