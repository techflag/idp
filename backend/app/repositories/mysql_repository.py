...
class MysqlWorkbenchRepository(WorkbenchRepository):
        ...
        with session_scope() as session:
            ...
            if includeFinalOutput:
                for step in reversed(steps):
                    if not step.executionRunId:
                        continue
                    prompt_run_model = session.get(PromptRunModel, step.executionRunId)
                    ...
                    if isinstance(structured_process_result, dict):
                        final_output = structured_process_result
                        break
                    ...
                    if isinstance(structured_extraction_result, dict):
                        final_output = structured_extraction_result
                        break
    ...
    def _get_latest_application_run_detail_for_task(self, taskId: str) -> ApplicationRunDetail | None:
        ...
        return ApplicationRunDetail(
            **self._to_application_run_summary(run_record, application_name).model_dump(),
            steps=[self._to_application_run_step_summary(step) for step in step_records],
            finalOutput=None,
        )
    ...
    def _apply_prompt_run_record(self, model: PromptRunModel, record: PromptRunRecord) -> None:
        ...
        persist_completed_result_inline = record.status not in {"completed", "needs_review"} or record.runPurpose == "summary"
        model.structured_extraction_result_json = (
            self._dumps_json(record.structuredExtractionResult) if persist_completed_result_inline else None
        )
        model.structured_process_result_json = (
            self._dumps_json(record.structuredProcessResult) if persist_completed_result_inline else None
        )
        model.structured_business_result_json = (
            self._dumps_json(record.structuredBusinessResult) if persist_completed_result_inline else None
        )
    ...
    def _load_structured_process_result(self, model: PromptRunModel) -> dict | None:
        ...
        artifact_payload = self._load_run_artifact_payload(model.task_id, model.id, "process_result")
        if isinstance(artifact_payload, dict):
            value = artifact_payload.get("structuredProcessResult")
            if isinstance(value, dict):
                return value
        artifact_payload = self._load_run_artifact_payload(model.task_id, model.id, "parse_result")
        if isinstance(artifact_payload, dict):
            value = artifact_payload.get("structuredProcessResult")
            if isinstance(value, dict):
                return value
    ...
    def _load_structured_extraction_result(self, model: PromptRunModel) -> dict | None:
        ...
        artifact_payload = self._load_run_artifact_payload(model.task_id, model.id, "parse_result")
        if isinstance(artifact_payload, dict):
            value = artifact_payload.get("structuredExtractionResult") or artifact_payload.get("extractionResult")
    ...
    def _load_structured_business_result(self, model: PromptRunModel) -> dict | None:
        ...
        for artifact_kind in ("parse_result", "process_result"):
            artifact_payload = self._load_run_artifact_payload(model.task_id, model.id, artifact_kind)
            if isinstance(artifact_payload, dict):
                value = artifact_payload.get("structuredBusinessResult")
                if isinstance(value, dict):
                    return value
        return None
