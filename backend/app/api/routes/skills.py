...
def sample_locate_and_extract(
    ...
    if payload.selectedCandidateId:
        ...
        confirmed_hint = _confirmed_evidence_hint_from_candidate(selected_candidate)
        if confirmed_hint:
            confirmed_hints = [
                item
                for item in (locator_profile.get("confirmedEvidenceHints") or [])
                if isinstance(item, dict)
            ]
            locator_profile = {
                **locator_profile,
                "confirmedEvidenceHints": [confirmed_hint, *confirmed_hints][:8],
                "selectedWindow": confirmed_hint.get("selectedWindow"),
            }
            locator_profile["derivedTerms"] = SemanticLocatorService._derive_locator_terms(locator_profile)
            trace["locator"]["profile"] = locator_profile
    elif locator_run.selected and locator_run.selected_candidates:
...
def _build_sample_locator_profile(
    *,
    query: str,
    output_preference: str,
    locator_instruction: str = "",
    expected_output: str = "",
) -> dict[str, Any]:
    ...
    if expected_output:
        content_contracts.append("用户提供了期望输出样例，字段信号可用于候选扫描。")
    profile = {
        "semanticSlot": query,
    ...
    profile["derivedTerms"] = SemanticLocatorService._derive_locator_terms(profile)
    return profile
...
def _sample_locate_candidate_payload(candidate: LocatorCandidate) -> dict[str, Any]:
    ...
    matched_terms = [str(item) for item in (payload.get("matchedTerms") or []) if str(item).strip()]
    matched_windows = [item for item in (payload.get("matchedWindows") or []) if isinstance(item, dict)]
    row_window = payload.get("rowWindow") if isinstance(payload.get("rowWindow"), dict) else None
    shape_signals = payload.get("shapeSignals") if isinstance(payload.get("shapeSignals"), dict) else {}
    uncertainties = [str(item) for item in (payload.get("uncertainties") or []) if str(item).strip()]
    return {
        ...
        "excerpt": _candidate_display_excerpt(candidate),
        "score": round(candidate.score, 3),
        "reasons": candidate.reasons[:8],
        "warnings": candidate.warnings[:8],
        "matchedTerms": matched_terms[:48],
        "matchedWindows": matched_windows[:4],
        "rowWindow": row_window,
        "shapeSignals": shape_signals,
        "uncertainties": uncertainties[:8],
        "deterministicScore": payload.get("deterministicScore") if payload.get("deterministicScore") is not None else round(candidate.score, 4),
...
def _candidate_display_excerpt(candidate: LocatorCandidate) -> str:
    payload = candidate.payload or {}
    window = payload.get("rowWindow") if isinstance(payload.get("rowWindow"), dict) else None
    if not window:
        windows = payload.get("matchedWindows")
        if isinstance(windows, list):
            window = next((item for item in windows if isinstance(item, dict)), None)
    if window:
        preview_rows = window.get("previewRows") if isinstance(window.get("previewRows"), list) else []
        lines = [
        ...
        if text:
            return text[:320]
    excerpt = str(candidate.excerpt or payload.get("excerpt") or "").strip()
    if _looks_like_raw_markup_or_inline_asset(excerpt):
        row_count = payload.get("rowCount") or candidate.row_count
        column_count = payload.get("columnCount") or candidate.column_count
        return f"{candidate.type or '候选'} · {row_count or 0} 行 · {column_count or 0} 列"
    return excerpt[:320]


def _looks_like_raw_markup_or_inline_asset(value: str) -> bool:
    text = str(value or "").strip().lower()
    return bool("<table" in text or "<img" in text or "data:image" in text or "base64," in text)


def _confirmed_evidence_hint_from_candidate(candidate: LocatorCandidate) -> dict[str, Any]:
    payload = candidate.payload or {}
    matched_terms = [str(item) for item in (payload.get("matchedTerms") or []) if str(item).strip()]
    matched_windows = [item for item in (payload.get("matchedWindows") or []) if isinstance(item, dict)]
    selected_window = payload.get("rowWindow") if isinstance(payload.get("rowWindow"), dict) else (matched_windows[0] if matched_windows else {})
    shape_signals = payload.get("shapeSignals") if isinstance(payload.get("shapeSignals"), dict) else {}
    hint: dict[str, Any] = {
        "source": "user_confirmed_candidate",
        "selectedTerms": matched_terms[:48],
        "matchedTerms": matched_terms[:48],
        "outputObjectType": candidate.type,
        "shapeSignals": shape_signals,
        "selectedWindow": selected_window,
        "selectionReason": "用户在样例制作中选择此候选；后续仅按信号和形态泛化，不固定旧节点 ID 或页码。",
    }
    if matched_windows:
        hint["matchedWindows"] = matched_windows[:4]
    source_refs: dict[str, Any] = {}
    if payload.get("source"):
        source_refs["source"] = payload.get("source")
    if payload.get("treeNodeId"):
        source_refs["sampleTreeNodeId"] = payload.get("treeNodeId")
    if payload.get("blockIds"):
        source_refs["sampleBlockIds"] = list(payload.get("blockIds") or [])[:20]
    if payload.get("pages"):
        source_refs["samplePages"] = list(payload.get("pages") or [])[:8]
    if source_refs:
        hint["sourceRefsForAuditOnly"] = source_refs
