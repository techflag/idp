...
const parsePanelResult = computed(() => (
  applicationRun.value
    ? applicationRunResultDetail.value ?? store.currentParseResultDetail
...
watch(
  ...
  async (loadKey) => {
    ...
    try {
      ...
      const result = buildMergedParseResultDetail(resultDetails) ?? resultDetails[resultDetails.length - 1] ?? null
